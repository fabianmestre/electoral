...
export default function PersonaForm() {
  ...
  const onCedula = (ced: string) => {
    ...
    const dup = db.personas.some((p) => p.cedula === ced.trim() && p.id !== personaModal?.personaId)
    if (dup) setHint({ tone: 'err', text: 'Cédula duplicada: ya existe un registro con esta cédula.' })
  }
  ...
  const save = () => {
    ...
    if (db.personas.some((p) => p.cedula === form.cedula.trim() && p.id !== personaModal?.personaId)) {
      notify('Cédula duplicada — registro rechazado', 'error')
      return
    }
    ...
    savePersona(input)
    notify(editing ? 'Ficha actualizada correctamente' : 'Simpatizante registrado 🎉', 'success')
  }
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <Field label="Líder asignado" required>
              <select className={inputCls} disabled={session?.rol !== 'admin'} value={form.liderId} onChange={(e) => patch({ liderId: e.target.value })}>
                {LIDERES_INFO.map((l) => (
