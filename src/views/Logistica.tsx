...
import { Activity, Car, CheckCircle2, ChevronLeft, ChevronRight, Clock, Users } from 'lucide-react'
import { useApp } from '../store'
import { LIDERES_INFO, esValido, nombreLider } from '../data'
...
const lugarDe = (p: { zona: string; comuna?: string; corregimiento?: string }) =>
...
export default function Logistica() {
  const { db, verPerfil, simularAvance, irADirectorio } = useApp()
  const [tab, setTab] = useState<'votos' | 'lideres' | 'transporte' | 'censo'>('votos')
  const [expanded, setExpanded] = useState<string | null>(null)

  const esperados = db.personas.filter(esValido).length
  const votosRegistrados = db.personas.filter((p) => p.votoRegistrado).length
  const pct = esperados ? Math.round((votosRegistrados / esperados) * 100) : 0

  const vehDisp = useMemo(
    () => db.personas.flatMap((p) => p.vehiculos.filter((v) => v.aDisposicion).map((v) => ({ ...v, propietario: p }))),
    [db],
  )
  ...
  db.personas.filter((p) => p.votoRegistrado).forEach((p) => {
    const h = p.votoHora.split(':')[0] || '?'
    porHora[h] = (porHora[h] || 0) + 1
  })
  const barrasHora = Object.keys(porHora).sort().map((h) => ({ label: `${h}:00`, value: porHora[h], color: '#6366f1' }))

  const votosDe = (lid: string) => db.personas.filter((p) => p.votoRegistrado && p.liderId === lid).length

  /* ---- Esperados vs reportados por territorio ---- */
  const porLugar: Record<string, { esperados: number; reportados: number }> = {}
  db.personas.filter((p) => esValido(p)).forEach((p) => {
  ...
  const bitacora = [...db.actividadDiaE].reverse().slice(0, 30)
  ...
  const firmesValidosTotal = db.personas.filter((p) => p.nivelVoto === 'Firme' && esValido(p)).length
  const firmesSinVotar = db.personas.filter((p) => p.nivelVoto === 'Firme' && esValido(p) && !p.votoRegistrado).length
  const lideresInactivos = LIDERES_INFO.filter((l) => {
    const ultima = db.actividadDiaE
      .filter((a) => a.liderId === l.id)
      .sort((a, b) => minutosDe(b.hora) - minutosDe(a.hora))[0]
    return !ultima || minutosDe(db.horaDiaE) - minutosDe(ultima.hora) > 120
  })
  const brechas = Object.entries(porLugar)
    .map(([lugar, { esperados, reportados }]) => ({ lugar, esperados, reportados, brecha: esperados - reportados }))
    .filter((b) => b.brecha > 0)
  ...
  db.personas.filter((p) => esValido(p) && p.nivelVoto === 'Firme' && !p.votoRegistrado).forEach((p) => {
    const lugar = lugarDe(p)
  ...
  /* ---- Trabajo de líderes ---- */
  const ranking = LIDERES_INFO.map((l) => {
    const votos = votosDe(l.id)
    const firmes = db.personas.filter((p) => p.liderId === l.id && p.nivelVoto === 'Firme' && esValido(p)).length
    const pctMeta = l.meta ? Math.round((votos / l.meta) * 100) : 0
    const acciones = db.actividadDiaE.filter((a) => a.liderId === l.id).length
    const activo = db.actividadDiaE.some(
      (a) => a.liderId === l.id && minutosDe(a.hora) >= minutosDe(db.horaDiaE) - 120,
    )
    return { ...l, votos, firmes, pctMeta, acciones, activo }
  }).sort((a, b) => b.pctMeta - a.pctMeta)

  const metaVsVotos = {
    groups: LIDERES_INFO.map((l) => nombreCorto(l)),
    series: [
      { label: 'Votos hoy', color: '#6366f1', values: LIDERES_INFO.map((l) => votosDe(l.id)) },
      ...
      <div className="bg-gradient-to-r from-blue-800 to-blue-600 rounded-2xl p-5 mb-4 text-white flex flex-wrap items-center gap-3">
        ...
        <div className="ml-auto flex items-center gap-3">
          <div className="inline-flex items-center gap-2 bg-white/15 rounded-lg px-3 py-2">
            ...
            <span className="font-bold tabular-nums">{db.horaDiaE}</span>
            <span className="text-blue-100 text-xs">hora simulada</span>
          </div>
          <button
              ...
              <div className="space-y-1 max-h-72 overflow-y-auto">
                ...
                {bitacora.map((a) => (
                  <div key={a.id} className="flex items-start gap-2 border-b border-slate-100 py-1.5">
                    <span className="text-[11px] font-mono text-slate-400 shrink-0 w-12">{a.hora}</span>
                    <Badge className={a.tipo === 'Apertura' ? 'bg-blue-100 text-blue-700' : 'bg-emerald-100 text-emerald-700 shrink-0'}>
                      {a.tipo}
                    </Badge>
                    <div className="min-w-0 text-sm">
                      <span className="font-medium text-slate-700">{nombreLider(a.liderId)}</span>{' '}
                            ...
                            vehiculos.map((v, i) => (
                              <tr key={i} className="bg-slate-50/70 border-b border-slate-100">
                                <td className="px-3 py-2 pl-8 text-sm">
                                  <button onClick={() => verPerfil(v.propietario.id)} className="text-blue-600 hover:underline text-left">
