...
export default function Gestiones() {
  ...
  const lista = useMemo(() => {
    let l = db.gestiones.slice().sort((a, b) => b.fecha.localeCompare(a.fecha))
    if (liderFilter !== 'all') l = l.filter((g) => getPersona(db, g.personaId)?.liderId === liderFilter)
    ...
    return l
  }, [db, liderFilter, cat, est, responsable, conMonto, desde, hasta, q])
  ...
  const barrasLider = LIDERES_INFO.map((l, i) => ({
    label: l.nombres,
    value: lista.filter((g) => getPersona(db, g.personaId)?.liderId === l.id).length,
    color: PALETA[i],
  }))
  ...
  const chips = useMemo(() => {
    ...
    if (liderFilter !== 'all') c.push({ key: 'lider', label: 'Líder', value: nombreLider(liderFilter) })
    ...
    return c
  }, [liderFilter, cat, est, responsable, conMonto, desde, hasta])
        ...
        <Card title="💰 Balance por persona (top 6)">
          <div className="space-y-2.5">
            ...
            {balances.map((b) => (
              <div key={b.p.id}>
                <div className="flex items-center justify-between text-xs mb-1">
                  <button onClick={() => verPerfil(b.p.id)} className="text-slate-600 hover:text-blue-600 truncate text-left flex-1">
              ...
              {lista.length === 0 && (
                <tr>
                  <td colSpan={8} className="px-3 py-10 text-slate-400 text-sm text-center">
                    Sin gestiones para los filtros aplicados.
                  </td>
                </tr>
              )}
              {lista.map((g) => {
                ...
                return (
                  <tr key={g.id} className="hover:bg-slate-50 border-b border-slate-100">
                    ...
                    <td className="px-3 py-2.5 whitespace-nowrap">
                      ...
                      <button onClick={() => eliminar(g.id)} className="text-xs text-red-500 hover:underline ml-2 inline-flex items-center gap-0.5">
