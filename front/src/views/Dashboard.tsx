...
export default function Dashboard() {
  const {
    ...
    irAComunicaciones, irADirectorio, irAGestiones, navigate,
  } = useApp()
  ...
  const data = useMemo(() => {
    ...
    simpatizantesApi.forEach((p) => val[validezDeApi(p)]++)
    const porMunicipio: Record<string, number> = {}
    simpatizantesApi.forEach((p) => (porMunicipio[p.municipio] = (porMunicipio[p.municipio] || 0) + 1))
    const cat: Record<string, number> = {}
    gestionesApi.forEach((g) => (cat[g.categoria] = (cat[g.categoria] || 0) + 1))
    const veh: Record<string, number> = { Moto: 0, Automóvil: 0, Camioneta: 0, Bus: 0 }
    simpatizantesApi.flatMap((p) => p.vehiculos).filter((v) => v.aDisposicion).forEach((v) => {
      if (v.tipo in veh) veh[v.tipo]++
    })
    const nivel: Record<string, number> = {}
    simpatizantesApi.forEach((p) => (nivel[p.nivelAcademico] = (nivel[p.nivelAcademico] || 0) + 1))
    return { val, porMunicipio, cat, veh, nivel }
  }, [simpatizantesApi, gestionesApi])

  const total = simpatizantesApi.length
  const validos = simpatizantesApi.filter(esValidoApi).length
  const firmesValidos = simpatizantesApi.filter((p) => p.nivelVoto === 'Firme' && esValidoApi(p)).length
  const gestionesPendientes = gestionesApi.filter((g) => g.estado === 'Pendiente' || g.estado === 'En Proceso').length
  const invertido = gestionesApi.reduce((s, g) => s + (Number(g.monto) || 0), 0)
  ...
  const barrasZonas = [
    ...COMUNAS.map((c) => ({ label: c, value: simpatizantesApi.filter((p) => p.comuna === c).length, color: '#0ea5e9' })),
    ...CORREGIMIENTOS.map((c) => ({ label: c, value: simpatizantesApi.filter((p) => p.corregimiento === c).length, color: '#10b981' })),
  ].sort((a, b) => b.value - a.value)
  const barrasLideres = lideresApi.map((l, i) => ({
    label: l.nombres,
    value: simpatizantesApi.filter((p) => p.liderId === l.id).length,
    color: PALETA[i % PALETA.length],
  })).sort((a, b) => b.value - a.value)
  const barrasBarrios = BARRIOS.map((b) => ({ label: b, value: simpatizantesApi.filter((p) => p.barrio === b).length, color: '#8b5cf6' })).sort((a, b) => b.value - a.value)
  const barrasPuestos = PUESTOS.map((p) => ({ label: p.id, value: simpatizantesApi.filter((x) => x.puesto === p.id).length, color: '#f59e0b' })).sort((a, b) => b.value - a.value)
  ...
  const monitoreoPadrinos = useMemo(
    () =>
      padrinosApi.filter((p) => p.activo).map((p) => {
        const lideres = lideresApi.filter((l) => l.padrinoId === p.id)
        const ids = new Set(lideres.map((l) => l.id))
        const fichas = simpatizantesApi.filter((x) => ids.has(x.liderId))
        const incompletas = fichas.filter((x) => !x.correo || !x.direccion).length
        const pctCompleto = fichas.length ? Math.round(((fichas.length - incompletas) / fichas.length) * 100) : 100
        return { ...p, lideres: lideres.length, fichas: fichas.length, incompletas, pctCompleto }
      }),
    [padrinosApi, lideresApi, simpatizantesApi],
  )
  ...
  return (
    <div>
      ...
      <Card title="🛡️ Monitoreo de padrinos (mesa de datos)" className="mb-4">
        <p className="text-xs text-slate-500 mb-3">Trabajo de cada padrino: líderes a cargo, fichas capturadas y completitud.</p>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[640px] text-left">
            <thead className="bg-slate-50">
              <tr>
                {['Padrino', 'Líderes', 'Fichas', 'Incompletas', 'Avance de completitud'].map((h) => (
            ...
            <tbody>
              {monitoreoPadrinos.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-3 py-8 text-slate-400 text-sm text-center">Sin padrinos activos todavía.</td>
                </tr>
              )}
      ...
      >
        <p className="text-[11px] text-amber-600 mb-2">
          Este bloque aún usa datos de demostración locales: el módulo Comunicaciones no está conectado al backend todavía.
        </p>
